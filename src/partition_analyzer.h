...
size_t process_partition_chunks(
    ...
    std::cout << "\r" << std::string(80, ' ') << "\r"; // Clear line
    std::cout << "Completed! Processed " << pairs_processed << " partition pairs. Found " 
              << valid_count << " valid mappings." << std::endl;
    
    // Sort valid mappings by their indices for consistent output
    std::sort(valid_mappings.begin(), valid_mappings.end(), 
...
size_t find_valid_partitions(const TransactionData& tx_data) {
    ...
    // Determine appropriate chunk size based on input/output sizes
    size_t elements_total = input_ids.size() + output_ids.size();
    size_t chunk_size = 1000; // Default
    
    // Adjust chunk size based on problem size
    if (elements_total > 16) {
        chunk_size = 100;
    } else if (elements_total > 12) {
        chunk_size = 500;
    }
